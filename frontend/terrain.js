...
const Terrain = (() => {
  ...
  async function fetchElevations(lat, lng, gridKm) {
    ...
    for (let start = 0; start < points.length; start += BATCH_SIZE) {
      ...
      // Let the user see progress instead of a frozen "Fetching..." message —
      // OpenTopoData's public API can legitimately take ~40-45s per 100-point
      // batch, so a full scan can take a good number of minutes.
      if (typeof showTerrainStatus === 'function') {
        showTerrainStatus(
          `⏳ Fetching real elevation data — batch ${batchIndex}/${totalBatches} ` +
          `(${realPointCount}/${start} real points so far). This can take several ` +
          `minutes total — each batch of 100 points can take ~40-45s.`,
          'loading'
        );
      }
